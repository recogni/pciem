...
#define pr_fmt(fmt) KBUILD_MODNAME ": iommu-stub: " fmt
                                 ...
                                 unsigned long action, void *data)
{
    ...
    if (rc && rc != -EALREADY) {
        pr_warn("fwspec_init(%s) failed: %d\n",
                dev_name(dev), rc);
        return NOTIFY_DONE;
    }
...
int pciem_iommu_stub_init(void)
{
    ...
    if (rc) {
        pr_err("software_node_register failed: %d\n", rc);
        return rc;
    }
    ...
    pr_info("registered (no-translation; for vfio binding only)\n");
    return 0;
